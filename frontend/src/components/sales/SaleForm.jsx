...
const SaleForm = ({ 
  ...
  const [saleType, setSaleType] = useState(formData.sale_type || 'non_prescription');
  ...
  useEffect(() => {
    ...
    const doctorFees = parseFloat(formData.doctor_fees) || 0;
    const newTotal = newSubtotal - discount + tax + doctorFees;
    setTotal(newTotal);
  }, [items, formData.discount, formData.tax, formData.doctor_fees]);
  ...
  const getMedicineDisplay = (medicine) => {
    return `${medicine.generic} | ${medicine.brand} | ${medicine.dosage} | ${medicine.strength}mg | Stock: ${medicine.stock}`;
  };
  ...
  const renderMedicineOption = (option) => {
    ...
    const stockStatus = medicine.stock > 5 ? 'In Stock' : medicine.stock > 0 ? 'Low Stock' : 'Out of Stock';
    const stockColor = medicine.stock > 5 ? 'text-green-600' : medicine.stock > 0 ? 'text-yellow-600' : 'text-red-600';
    ...
    return (
      <div className="flex flex-col py-0.5">
        ...
        <div className="flex flex-wrap gap-1 mt-0.5">
          ...
          <span className={`text-xs ${stockColor} bg-gray-50 px-1.5 py-0.5 rounded`}>
  ...
  const handleFormSubmit = (e) => {
    ...
    const submitData = {
      ...
      sale_type: formData.sale_type || 'non_prescription',
    ...
    <form onSubmit={handleFormSubmit} className="space-y-6">
      <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-800">
            {isEditing ? t('sales.edit') : t('sales.new')}
          </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => handleSaleTypeChange('non_prescription')}
              className={`p-4 rounded-xl border-2 transition-all duration-200 text-center ${
                saleType === 'non_prescription'
                  ? 'border-blue-500 bg-blue-50 shadow-md'
                  : 'border-gray-200 hover:border-gray-300'
              }`}
            >
              <FiUser className={`mx-auto text-2xl ${saleType === 'non_prescription' ? 'text-blue-600' : 'text-gray-400'}`} />
              <p className={`text-sm font-medium mt-1 ${saleType === 'non_prescription' ? 'text-blue-700' : 'text-gray-600'}`}>
                {t('sales.fields.nonPrescription')}
              </p>
              <p className="text-xs text-gray-400">OTC</p>
            </button>
            <button
              type="button"
              onClick={() => handleSaleTypeChange('prescription')}
              className={`p-4 rounded-xl border-2 transition-all duration-200 text-center ${
            ...
            >
              <FiClipboard className={`mx-auto text-2xl ${saleType === 'prescription' ? 'text-purple-600' : 'text-gray-400'}`} />
              <p className={`text-sm font-medium mt-1 ${saleType === 'prescription' ? 'text-purple-700' : 'text-gray-600'}`}>
                {t('sales.fields.prescription')}
              </p>
              <p className="text-xs text-gray-400">{t('sales.fields.prescription')}</p>
            </button>
          </div>
        </div>
        <div>
      ...
      {saleType === 'prescription' && (
        <div className="bg-purple-50 rounded-xl p-4 border border-purple-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label={t('sales.fields.prescriptionNumber')}
            ...
            />
            <Input
              label={t('sales.fields.doctorName')}
              name="doctor_name"
              value={formData.doctor_name || ''}
              onChange={handleInputChange}
              placeholder={t('forms.enterName')}
              required
              icon={<FiUserPlus className="text-gray-400" size={18} />}
            />
            <Input
              label={t('sales.fields.doctorFees')}
              name="doctor_fees"
              type="number"
              step="0.01"
              min="0"
            ...
            />
            <Input
              label={t('sales.fields.patientName')}
              name="patient_name"
            ...
            />
            <Input
              label={t('sales.fields.patientPhone')}
              name="patient_phone"
          ...
          <div className="mt-3">
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              {t('sales.fields.prescriptionImage')}
            </label>
        ...
        <div className="grid grid-cols-12 gap-3 items-end">
          ...
          <div className="col-span-3 md:col-span-2">
            <Button
              ...
              onClick={addItem}
              icon={<FiPlus size={18} />}
              className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 shadow-md hover:shadow-lg transition-all py-2.5"
            >
              {t('common.add')}
            </Button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <div className="h-full bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl p-4 flex items-center justify-between shadow-lg">
            <div>
              <p className="text-green-100 text-sm font-medium">{t('common.total')}</p>
              <p className="text-white text-2xl font-bold">${total.toFixed(2)}</p>
              {saleType === 'prescription' && parseFloat(formData.doctor_fees || 0) > 0 && (
                <p className="text-green-200 text-xs">Includes {t('sales.fields.doctorFees')}: ${parseFloat(formData.doctor_fees || 0).toFixed(2)}</p>
              )}
            </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows="2"
          className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 resize-none bg-gray-50/50 hover:bg-white"
        />
      </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-gray-200">
        ...
        <Button
          ...
          variant="success"
          loading={loading}
          className="flex-1 py-3 text-base bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 shadow-md hover:shadow-lg transition-all"
          disabled={items.length === 0}
          icon={<FiShoppingCart size={18} />}
        >
          {isEditing ? t('common.update') : t('sales.new')}
        </Button>
      </div>
